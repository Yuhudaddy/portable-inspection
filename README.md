...
5. Tap the export button (⬆, bottom right); pick 單頁 (current form PDF) or 整份 (complete record PDF), and share the result to LINE or Files. JSON is the canonical structured file (schema 1.7; the rebar-cage review carries `mode` and 13 fixed `parts`, the guide-wall review carries `axis_no` and each item's design value / design bar size and spacing, numeric items carry `unit`, and `construction_plan` carries the plan cover fields) and can be imported back through the menu's **匯入** item on the two diaphragm-wall tools; older 1.x files are migrated on import.
