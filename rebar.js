...
const statusSegmented = (name, selected, attrs) => `<div class="glass-segmented" role="radiogroup" aria-labelledby="${name}-label">${["合格", "不合格", "不適用"].map(value => { const kind = value === "合格" ? "pass" : value === "不合格" ? "fail" : "na"; return `<label><input type="radio" name="${name}" value="${value}" aria-label="${value}" ${value === selected ? "checked" : ""} ${attrs} /><span class="is-${kind}">${SEGMENT_ICONS[kind]}</span></label>`; }).join("")}</div>`;

function createMember(source = {}) { return { id: source.id || "", type: source.type || "柱", grid: source.grid || "", strength: source.strength || "SD420", width: source.width || "", height: source.height || "", cover: source.cover || "", note: source.note || "", reviewResult: source.reviewResult || "待確認", bars: (source.bars?.length ? source.bars.map(bar => ({ kind: bar.kind || "主筋", size: bar.size || "", count: bar.count || "", spacing: bar.spacing || "", note: bar.note || "" })) : [{ kind: "主筋", size: "", count: "", spacing: "", note: "" }]), checks: { ...(source.checks || {}) }, detailChecks: { ...(source.detailChecks || {}) } }; }
...
function memberReviewResult(member) {
  const records = [...Object.values(member.checks || {}), ...Object.values(member.detailChecks || {})];
  if (records.some(record => record.result === "不合格")) return "不合格";
  if (member.reviewResult && member.reviewResult !== "待確認") return member.reviewResult;
  if (records.length && records.every(record => ["合格", "不適用"].includes(record.result))) return "合格";
  return member.reviewResult || "待確認";
}

function barSummary(member) {
  return member.bars.map(bar => {
    const size = barSizeLabel(bar.size) || "未選號數";
    const count = bar.count ? ` × ${bar.count}支` : "";
    const spacing = bar.spacing ? ` @${bar.spacing}cm` : "";
    return `${bar.kind || "鋼筋"} ${size}${count}${spacing}`;
  }).join("；");
}

function renderBars(member, memberIndex) {
  return `<div class="bar-entry-wrap"><div class="bar-entry-head"><div><strong>配筋組合</strong><span>同一構件可建立多組主筋、箍筋、水平筋或補強筋</span></div><button class="add-button compact-button" type="button" data-add-bar="${memberIndex}">＋新增一組</button></div><div class="bar-entry-list"><div class="bar-entry bar-entry-header"><span>類別</span><span>鋼筋號數</span><span>支數</span><span>間距（cm）</span><span>備註／位置</span><span></span></div>${member.bars.map((bar, barIndex) => `<div class="bar-entry"><label class="field"><span>類別</span><select data-bar-field="kind" data-member-index="${memberIndex}" data-bar-index="${barIndex}">${BAR_KINDS.map(item => `<option value="${item}" ${item === bar.kind ? "selected" : ""}>${item}</option>`).join("")}</select></label><label class="field"><span>鋼筋號數</span><select data-bar-field="size" data-member-index="${memberIndex}" data-bar-index="${barIndex}">${barSizeOptions(bar.size, "選擇號數")}</select></label><label class="field"><span>支數</span><input type="number" min="0" step="1" data-bar-field="count" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.count)}" placeholder="例如：12" /></label><label class="field"><span>間距（cm）</span><input type="number" min="0" step="0.5" data-bar-field="spacing" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.spacing)}" placeholder="例如：15" /></label><label class="field bar-note"><span>備註／位置</span><input type="text" data-bar-field="note" data-member-index="${memberIndex}" data-bar-index="${barIndex}" value="${esc(bar.note)}" placeholder="例如：上層、四面配置" /></label>${member.bars.length > 1 ? `<button class="bar-remove" type="button" data-remove-bar="${memberIndex}" data-bar-index="${barIndex}">移除</button>` : ""}</div>`).join("")}</div></div>`;
}

function renderMembers() {
  $("#member-count").textContent = `${state.members.length} 個構件`;
  const statusOptionsForMember = selected => STATUS_OPTIONS.map(item => `<option value="${item}" ${item === selected ? "selected" : ""}>${item}</option>`).join("");
  $("#member-list").innerHTML = `<div class="member-table-wrap"><table class="member-matrix"><thead><tr><th>項次</th><th>構件類型</th><th>構件編號</th><th>軸線／位置</th><th>強度</th><th>寬／厚<br />cm</th><th>高<br />cm</th><th>保護層<br />cm</th><th>配筋摘要</th><th>結果</th><th>操作</th></tr></thead><tbody>${state.members.map((member, index) => { ensureMember(member); const result = memberReviewResult(member); const commonFields = `<div class="member-detail-fields"><label class="field"><span>構件類型</span><select data-member-field="type" data-member-index="${index}">${MEMBER_TYPES.map(type => `<option value="${type}" ${member.type === type ? "selected" : ""}>${type}</option>`).join("")}</select></label><label class="field"><span>構件編號</span><input type="text" data-member-field="id" data-member-index="${index}" value="${esc(member.id)}" placeholder="例如：C1-01" /></label><label class="field"><span>軸線／位置</span><input type="text" data-member-field="grid" data-member-index="${index}" value="${esc(member.grid)}" placeholder="例如：A-1／B～C" /></label><label class="field"><span>鋼筋強度</span><input type="text" data-member-field="strength" data-member-index="${index}" value="${esc(member.strength)}" placeholder="例如：SD420" /></label><label class="field"><span>設計寬度／厚度（cm）</span><input type="text" data-member-field="width" data-member-index="${index}" value="${esc(member.width)}" placeholder="例如：30" /></label><label class="field"><span>設計高度（cm）</span><input type="text" data-member-field="height" data-member-index="${index}" value="${esc(member.height)}" placeholder="例如：60" /></label><label class="field"><span>設計保護層（cm）</span><input type="text" data-member-field="cover" data-member-index="${index}" value="${esc(member.cover)}" placeholder="例如：4" /></label></div>`; return `<tr class="member-grid-row" data-member-row="${index}"><td>${index + 1}</td><td><select data-member-field="type" data-member-index="${index}" aria-label="第 ${index + 1} 筆構件類型">${MEMBER_TYPES.map(type => `<option value="${type}" ${member.type === type ? "selected" : ""}>${type}</option>`).join("")}</select></td><td><input type="text" data-member-field="id" data-member-index="${index}" value="${esc(member.id)}" placeholder="C1-01" aria-label="第 ${index + 1} 筆構件編號" /></td><td><input type="text" data-member-field="grid" data-member-index="${index}" value="${esc(member.grid)}" placeholder="A-1" aria-label="第 ${index + 1} 筆軸線位置" /></td><td><input type="text" data-member-field="strength" data-member-index="${index}" value="${esc(member.strength)}" placeholder="SD420" aria-label="第 ${index + 1} 筆鋼筋強度" /></td><td><input type="text" data-member-field="width" data-member-index="${index}" value="${esc(member.width)}" placeholder="30" aria-label="第 ${index + 1} 筆寬度" /></td><td><input type="text" data-member-field="height" data-member-index="${index}" value="${esc(member.height)}" placeholder="60" aria-label="第 ${index + 1} 筆高度" /></td><td><input type="text" data-member-field="cover" data-member-index="${index}" value="${esc(member.cover)}" placeholder="4" aria-label="第 ${index + 1} 筆保護層" /></td><td class="member-bars-summary" title="${esc(barSummary(member))}">${esc(barSummary(member))}</td><td><select class="member-result" data-member-review="${index}" aria-label="第 ${index + 1} 筆複核結果">${statusOptionsForMember(result)}</select></td><td class="member-actions"><button class="secondary-button compact-button" type="button" data-toggle-member="${index}">編輯配筋</button>${deleteButtonHtml(index)}</td></tr><tr class="member-detail-row" id="member-detail-${index}" hidden><td colspan="11"><div class="member-detail-editor"><div class="member-detail-heading"><strong>第 ${index + 1} 筆｜${esc(member.type)}｜${esc(member.id || "未編號")}</strong><div><button class="secondary-button compact-button" type="button" data-duplicate-member="${index}">複製構件</button></div></div>${commonFields}${renderBars(member, index)}<label class="field"><span>圖說／特殊條件備註</span><input type="text" data-member-field="note" data-member-index="${index}" value="${esc(member.note)}" placeholder="例如：開口補強、增築部、特殊續接" /></label></div></td></tr>`; }).join("")}</tbody></table></div>${state.members.length ? "" : `<div class="empty-state">尚無構件，請按「＋新增一筆」。</div>`}`;
  renderSelectors();
}
...
function renderCheckCard(check, index, collection, record) { const actual = record?.actual || ""; const result = record?.result || "待確認"; const name = `check-${collection}-${esc(check[0])}-result`; return `<article class="check-card ${result === "不合格" ? "is-failed" : result !== "待確認" ? "is-passed" : "is-pending"}"><div><h4>${index + 1}. ${esc(check[1])}</h4><p>${esc(check[2])}</p></div><div class="check-card-fields"><label class="field"><span>紀錄／實測</span><input type="text" data-check-actual="${collection}" data-check-id="${esc(check[0])}" value="${esc(actual)}" placeholder="${esc(check[3] || "填寫現場結果")}" /></label><div class="field result-field"><span id="${name}-label">結果</span>${statusSegmented(name, result, `data-check-result="${collection}" data-check-id="${esc(check[0])}"`)}</div></div></article>`; }
...
function renderPrint() {
  ...
  const memberRows = state.members.map((m, i) => { const bars = m.bars.map(bar => esc(`${bar.kind || "鋼筋"} ${barSizeMark(bar.size)}${bar.count ? ` × ${bar.count}支` : ""}${bar.spacing ? ` @${bar.spacing}cm` : ""}${bar.note ? `（${bar.note}）` : ""}`)).join("<br />"); return `<tr><td>${i + 1}</td><td>${printValue(m.type)}</td><td>${printValue(m.id)}</td><td class="text-left">${printValue(m.grid)}</td><td>${printValue(m.strength)}</td><td>${printValue(m.width)} × ${printValue(m.height)}</td><td class="text-left">${bars}</td><td>${printValue(m.cover)}</td><td>${printValue(memberReviewResult(m))}</td><td class="text-left">${printValue(m.note)}</td></tr>`; }).join("");
  $("#print-template-overview").innerHTML = `${printHeader("鋼筋工程查驗表", "01–02")}<section class="print-section"><h2>01｜工程概要</h2><div class="print-meta-grid three">${overviewMeta.map(([label, value]) => `<div><span>${label}</span><strong>${printValue(value)}</strong></div>`).join("")}</div></section><section class="print-section"><h2>02｜配筋明細</h2><table class="print-table rebar-detail-print-table"><thead><tr><th>項次</th><th>類型</th><th>構件編號</th><th>軸線／位置</th><th>強度</th><th>尺寸<br />cm</th><th>配筋明細</th><th>保護層<br />cm</th><th>結果</th><th>備註</th></tr></thead><tbody>${memberRows || `<tr><td colspan="10" class="print-empty">尚無構件</td></tr>`}</tbody></table></section>${printFooter()}`;
...
function loadExample() {
  ...
  state = createState();
  state.overview = { project: "Example Construction Project", contractor: "Example Rebar Co.", date: "2026-08-12", inspectionDate: "2026-08-12", reviewer: "Site Engineer", floor: "3F", drawing: "S-203 Rev.2", stage: "綁紮完成" };
...
function handleEvent(event) {
  ...
  if (target.matches("[data-member-review]")) { const member = state.members[Number(target.dataset.memberReview)]; if (member) member.reviewResult = target.value; }
  if (target.matches("[data-bar-field]")) { const member = state.members[Number(target.dataset.memberIndex)]; if (member?.bars[Number(target.dataset.barIndex)]) { member.bars[Number(target.dataset.barIndex)][target.dataset.barField] = target.value; const summary = document.querySelector(`[data-member-row="${target.dataset.memberIndex}"] .member-bars-summary`); if (summary) { summary.textContent = barSummary(member); summary.title = barSummary(member); } } }
  ...
  if (target.matches("#active-member, #detail-member")) { state.activeMember = Number(target.value) || 0; renderPlacement(); renderDetail(); renderSelectors(); }
}
