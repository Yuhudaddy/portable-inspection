...
Production: **https://portable-inspection.pages.dev/** — a Cloudflare Pages project connected to this repository (no build step, output directory `/`), redeployed on every push to `main`. `_headers` sets `Cache-Control: no-cache` on `sw.js` and the manifest. Internal links are extension-less (`./diaphragm-wall`, not `.html`) because Pages redirects `*.html` URLs. The former GitHub Pages site (`yuhudaddy.github.io/portable-inspection`) still builds from the same branch but only shows a "moved" notice on its home page. Retire it after the transition period (planned: on or after 2026-10-01, once field users have re-added the home-screen app) with `gh api -X DELETE repos/Yuhudaddy/portable-inspection/pages`; the repository itself stays as the source for Cloudflare Pages.
